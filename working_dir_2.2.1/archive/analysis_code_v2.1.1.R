...
tryCatch(for (i in 1:length(run_file_total_unknown$CPR)) {
  run_file_total_unknown$test10[i] <- ifelse(as.numeric(run_file_total_unknown$CPR[i])>=as.numeric(45), ".","FAIL")
}, error=function(e){"NA"})

#Limit of quantification test (Test 11)
tryCatch(for (i in 1:length(run_file_total_unknown$CPR)) {
  run_file_total_unknown$test11[i] <- ifelse(as.numeric(run_file_total_unknown$CPR[i])>=as.numeric(100), ".","FAIL")
}, error=function(e){"NA"})
...
#Limit of detection test (Test 10)
tryCatch(for (i in 1:length(run_file_toxin_unknown_undetermined$CPR)) {
  run_file_toxin_unknown$test10[i] <- ifelse(as.numeric(run_file_toxin_unknown$CPR[i])>=as.numeric(45), ".","FAIL")
}, error=function(e){"NA"})

#Limit of quantification test (Test 11)
tryCatch(for (i in 1:length(run_file_toxin_unknown$CPR)) {
  run_file_toxin_unknown$test11[i] <- ifelse(as.numeric(run_file_toxin_unknown$CPR[i])>=as.numeric(100), ".","FAIL")
}, error=function(e){"NA"})
tryCatch(test10_11_result_toxin <- run_file_toxin_unknown %>% select(Well_Position, Sample_Name, Target_Name, Task, CT, Ct_Mean, Ct_SD, CPR, CPR_mean, Stdev_CPR, RSD, test10, test11), error=function(e){"NA"})

tryCatch(write.csv(
  test10_11_result_toxin, file='output/mcye/tests/test10_11_result_mcye.csv', row.names=FALSE), error=function(e){"NA"})
...
tryCatch(for (i in 1:length(run_file_sxta_unknown$CPR)) {
  run_file_sxta_unknown$test10[i] <- ifelse(as.numeric(run_file_sxta_unknown$CPR[i])>=as.numeric(45), ".","FAIL")
}, error=function(e){"NA"})

#Limit of quantification test (Test 11)
tryCatch(for (i in 1:length(run_file_sxta_unknown$CPR)) {
  run_file_sxta_unknown$test11[i] <- ifelse(as.numeric(run_file_sxta_unknown$CPR[i])>=as.numeric(100), ".","FAIL")
}, error=function(e){"NA"})
tryCatch(test10_11_result_sxta <- run_file_toxin_unknown %>% select(Well_Position, Sample_Name, Target_Name, Task, CT, Ct_Mean, Ct_SD, CPR, CPR_mean, Stdev_CPR, RSD, test10, test11), error=function(e){"NA"})

tryCatch(write.csv(
  test10_11_result_sxta, file='output/sxta/tests/test10_11_result_sxta.csv', row.names=FALSE), error=function(e){"NA"})
